...
export function createPanel() {
    ...
    flashBand = h('div#vte-code-band.vte-code-band', {
        'aria-hidden': 'true', style: 'display:none',
    });
    const surface = h('div.vte-code-surface', {}, [flashBand, searchLayer, hl, ta]);
...
/* ============================================================
   ПОДСВЕТКА УЧАСТКА, КОТОРЫЙ ТОЛЬКО ЧТО ИЗМЕНИЛСЯ
   Вызывается из index.js после записи в CSS.
============================================================ */
...
export function revealRange(from, to, opts = {}) {
    ...
    const text = ta.value;
    const a = Math.max(0, Math.min(text.length, from | 0));
    const b = Math.max(a, Math.min(text.length, to | 0));

    const startLine = text.slice(0, a).split('\n').length;
    const endLine = text.slice(0, b).split('\n').length;
    const span = endLine - startLine + 1;

    flashLines = { startLine, endLine };

    // Ставим участок по центру, если он влезает целиком
    const lh = lineHeight();
    const free = Math.max(0, ta.clientHeight - span * lh);
    ta.scrollTop = Math.max(0, (startLine - 1) * lh - free / 2);

    markTouchedLines(startLine, endLine);
    positionBand();
    scheduleSync();
...
export function clearFlash() {
    clearTimeout(flashTimer);
    flashLines = null;
    if (flashBand) flashBand.style.display = 'none';
    gutter?.querySelectorAll('.vte-code-ln-touched').forEach(n =>
        n.classList.remove('vte-code-ln-touched'));
}

function markTouchedLines(from, to) {
    gutter?.querySelectorAll('.vte-code-ln-touched').forEach(n =>
        n.classList.remove('vte-code-ln-touched'));
    for (let n = from; n <= to; n++) {
        gutter?.querySelector(`[data-line="${n}"]`)?.classList.add('vte-code-ln-touched');
    }
}

/** Полоска подсветки живёт в координатах прокрутки, поэтому её надо двигать */
function positionBand() {
    if (!flashBand || !ta) return;
    if (!flashLines) { flashBand.style.display = 'none'; return; }

    const lh = lineHeight();
    const padTop = 8; // совпадает с padding в .vte-code-input
    const top = padTop + (flashLines.startLine - 1) * lh - ta.scrollTop;
    const height = (flashLines.endLine - flashLines.startLine + 1) * lh;

    flashBand.style.display = 'block';
    flashBand.style.top = `${Math.round(top)}px`;
    flashBand.style.height = `${Math.round(height)}px`;
}
...
function handleInput() {
    ...
    clearFlash();          // ручной ввод снимает подсветку «вот тут изменилось»
    scheduleRender();
    markDirty(true);
    scheduleSnapshot();
...
/**
 * Запас прокрутки в конце слоя.
 *
 * У textarea есть собственные полосы прокрутки: горизонтальная забирает
 * 10-17 px высоты, вертикальная столько же ширины. Из-за этого textarea
 * прокручивается дальше, чем слои подсветки — у них полос нет, значит нет
 * и запаса. scrollTop у слоя упирается в предел, текст остаётся на месте,
 * а каретка с выделением уезжают на строку вверх.
 */
function scrollSpacer() {
    return h('div.vte-code-spacer', { 'aria-hidden': 'true' });
}
...
/**
 * Прокрутить слой вслед за textarea.
 * Если слою не хватило запаса и он не доехал — доводим сдвигом.
 * Сдвиг не зависит от overflow вообще, поэтому промаха уже не будет.
 */
function applyScroll(layer, top, left) {
    if (!layer) return;

    layer.scrollTop = top;
    layer.scrollLeft = left;

    const dy = top - layer.scrollTop;
    const dx = left - layer.scrollLeft;
    layer.style.transform = (dx || dy) ? `translate(${-dx}px, ${-dy}px)` : '';
}
...
function syncScroll() {
    ...
    if (!plainMode) applyScroll(hl, top, left);
    applyScroll(searchLayer, top, left);
